  ...
  >
    ...
    >
      <el-row class="flex justify-between">
        <el-form-item label="联系人" prop="name" class="mr-6">
...
export default {
    ...
    return {
      ...
      rules: {
        company: [
          {
            required: true,
            message: "请输入当前供应商公司地址",
            trigger: "blur",
          },
          {
            min: 1,
            max: 50,
            message: "公司地址应当在 50 个字符",
            trigger: "blur",
          },
        ],
        phone: [
  ...
  computed: {
    ...mapGetters(["getDialogFormVisible", "getSupplierTotal", "getPageSize"]),
    dialogFormVisible: {
      get() {
  ...
  methods: {
    ...
    ...mapActions(["fetchSource", "createSupplier"]),
    // 提交表单 - 添加新供应商
    async submitForm(formName) {
      await this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          await sleep();
          this.setPage(
            Math.ceil(this.getSupplierTotal / this.getPageSize) || 1,
          );
