...
const { mapGetters, mapMutations, mapActions } =
  createNamespacedHelpers("orderArea");
export default {
  name: "OrderFactory",
  async created() {
    this.users = await this.fetchUsers().then((users) =>
      users.map((user) => ({
        value: user.id,
        label: user.username,
      })),
  ...
  computed: {
    ...mapGetters(["getDialogFormVisible", "getOrderTotal", "getPageSize"]),
    dialogFormVisible: {
      get() {
  ...
  methods: {
    ...
    async changeProduct(id) {
      if (id) {
        this.warehouses = await this.fetchProductInventory(id).then(
          (warehouses) =>
            warehouses.map((warehouse) => ({
              value: warehouse.id,
              label: warehouse.location,
    ...
    async changeWarehouse(id) {
      if (id) {
        this.products = await this.fetchWarehouseInventory(id).then(
          (products) =>
            products.map((product) => ({
              value: product.id,
              label: product.name + "-" + product.model,
