...
export default {
  ...
  computed: {
    ...mapGetters(["getDialogFormVisible", "getWarehouseTotal", "getPageSize"]),
    dialogFormVisible: {
      get() {
  ...
  methods: {
    ...
    ...mapActions(["fetchSource", "createWarehouse"]),
    // 提交表单 - 创建新产品
    async submitForm(formName) {
      await this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          await sleep();
          this.setPage(
            Math.ceil(this.getWarehouseTotal / this.getPageSize) || 1,
          );
