      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button @click="handelLocationChange(scope.row)" type="text"
              >修改</el-button
            >
            <el-button @click="handleWarehouseDelete(scope.row)" type="text"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
const { mapGetters, mapMutations, mapActions } =
  createNamespacedHelpers("warehouseArea");
export default {
  name: "WarehouseTable",
  async created() {
  ...
  methods: {
    ...
    ...mapActions(["fetchSource", "fetchWarehouse", "deleteWarehouse"]),
    // 处理修改地址
