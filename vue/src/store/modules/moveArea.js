...
export default {
    ...
    getters: {
        ...
        getMoveTotal: state => state.moveTotal,
    ...
    mutations: {
        ...
        setMoveTotal: (state, payload) => { state.moveTotal = payload },
        setMove: (state, payload) => { state.move = payload },
    ...
    actions: {
        ...
        async fetchSource({ state, commit }) {   
            const response = await api.get(`/api/move?page=${state.page}&pageSize=${state.pageSize}`,{ token: state.token })
            commit("setMoveTotal", response.moveTotal)
            return response
        }
    }
}