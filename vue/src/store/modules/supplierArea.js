...
export default {
  ...
  mutations: {
    ...
    setSupplierTotal: (state, payload) => {
      state.supplierTotal = payload;
    },
  ...
  actions: {
    ...
    async fetchSource({ state, commit }) {
      const response = await api.get(
        `/api/supplier?page=${state.page}&pageSize=${state.pageSize}`,
    ...
    async fetchSupplier(context, payload) {
      const response = await api.get(`/api/supplier/${payload}`, {
        token: localStorage.getItem("token"),
      });
    ...
    async updateSupplier(context, payload) {
      await api.patch(`/api/supplier/${payload.id}`, payload.data, {
        token: localStorage.getItem("token"),
      });
    },
    // 请求接口 - 删除供应商
    async deleteSupplier(context, payload) {
      await api.delete(`/api/supplier/${payload}`, {
        token: localStorage.getItem("token"),
      });
    },
  },
