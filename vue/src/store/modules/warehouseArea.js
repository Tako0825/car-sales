...
export default {
  ...
  mutations: {
    ...
    setWarehouseTotal: (state, payload) => {
      state.warehouseTotal = payload;
    },
    ...
    setDialogEditVisible: (state, payload) => {
  ...
  actions: {
    ...
    async fetchSource({ state, commit }) {
      const response = await api.get(
        `/api/warehouse?page=${state.page}&pageSize=${state.pageSize}`,
    ...
    async fetchWarehouse(context, payload) {
      const response = await api.get(`/api/warehouse/${payload}`, {
        token: localStorage.getItem("token"),
      });
    ...
    async updateWarehouse(context, payload) {
      await api.patch(`/api/warehouse/${payload.id}`, payload.data, {
        token: localStorage.getItem("token"),
      });
    },
    // 请求接口 - 删除仓库信息
    async deleteWarehouse(context, payload) {
      await api.delete(`/api/warehouse/${payload}`, {
        token: localStorage.getItem("token"),
      });
    },
  },
