  ...
  actions: {
    ...
    async fetchSource({ state, commit }) {
      const response = await api.get(
        `/api/order?page=${state.page}&pageSize=${state.pageSize}`,
    ...
    async fetchProductInventory(context, payload) {
      return await api.get(`/api/product/${payload}/inventory`, {
        token: localStorage.getItem("token"),
      });
    },
    // 请求接口 - 获取相应仓库的库存
    async fetchWarehouseInventory(context, payload) {
      return await api.get(`/api/warehouse/${payload}/inventory`, {
        token: localStorage.getItem("token"),
      });
    },
    // 请求接口 - 创建订单
    ...
    async deleteOrder(context, payload) {
      await api.delete(`/api/order/${payload}`, {
        token: localStorage.getItem("token"),
      });
    },
  },
