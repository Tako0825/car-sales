...
export default {
  ...
  mutations: {
    ...
    setRegisterFormVisible: (state, payload) => {
      state.registerFormVisible = payload;
    },
  },
  actions: {
    // 请求接口 - 分页获取用户信息
    async fetchSource({ state, commit }) {
      const response = await api.get(
        `/api/user?page=${state.page}&pageSize=${state.pageSize}`,
    ...
    async updateUser(context, payload) {
      await api.patch(`/api/user/${payload.id}`, payload.data, {
        token: localStorage.getItem("token"),
      });
    },
    // 请求接口 - 修改指定用户头像
    async updateAvatar(context, payload) {
      await api.patch(`/api/user/${payload.id}/avatar`, payload.data, {
        token: localStorage.getItem("token"),
      });
    },
    // 请求接口 - 注册用户
    ...
    async isPhoneExisted(context, payload) {
      const response = await api.get(`/api/user/phone/${payload}`, {
        token: localStorage.getItem("token"),
      });
      return response;
    },
    ...
    async deleteUser(context, payload) {
      await api.delete(`/api/user/${payload}`, {
        token: localStorage.getItem("token"),
      });
    },
    // 请求接口 - 查询用户详情
    async fetchUserDetail({ commit }, payload) {
      const response = await api.get(`/api/user/${payload}/order`, {
        token: localStorage.getItem("token"),
      });
